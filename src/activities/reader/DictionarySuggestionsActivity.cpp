...
void DictionarySuggestionsActivity::onExit() {
  Activity::onExit();
  // Explicitly clear and shrink the vector to free RAM immediately on exit.
  suggestions.clear();
  suggestions.shrink_to_fit();
}

void DictionarySuggestionsActivity::findSuggestions() {
  // The similarity scan needs the sparse index in RAM; load it on demand so
  // entering from a stale state still works (it's fast from the SD cache).
  ...
  // The dictionary index is loaded into RAM here. Explicitly release it back
...
void DictionarySuggestionsActivity::render(RenderLock&&) {
  ...
  renderer.drawText(UI_12_FONT_ID, margin, currentY, titleStr.c_str(), true, EpdFontFamily::BOLD);
