...
void LookedUpWordsActivity::onEnter() {
  Activity::onEnter();
  // Trigger an initial render to show "Loading", then load data in the loop
  isLoading = true;
  requestUpdate();
}
...
void LookedUpWordsActivity::onExit() {
  Activity::onExit();
  // Ensure historical data vectors are fully wiped from memory on exit
  words.clear();
  words.shrink_to_fit();
}
...
void LookedUpWordsActivity::deleteSelectedWord() {
  if (words.empty() || selectedIndex < 0 || selectedIndex >= static_cast<int>(words.size())) return;
  ...
  if (selectedIndex >= static_cast<int>(words.size()) && selectedIndex > 0) {
    selectedIndex--;
  }
...
void LookedUpWordsActivity::render(RenderLock&&) {
  ...
  // Draw Header
  renderer.drawText(UI_12_FONT_ID, margin, currentY, tr(STR_LOOKUP_HISTORY), true, EpdFontFamily::BOLD);
  ...
  if (confirmingDelete) {
    ...
    char buf[128];
    snprintf(buf, sizeof(buf), "%s '%s'?", tr(STR_DELETE_WORD_PROMPT), words[selectedIndex].c_str());
    ...
    // No "Delete" tag in the next/right hint slot — on several button
    // mappings the front left/right buttons do nothing in this view, so
    // labeling them would mislead. Delete still works via the side rocker's
    // Button::Right press (handled in loop()).
